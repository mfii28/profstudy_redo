...
async def get_generic_setting(
    key: str,
    db: AsyncSession = Depends(get_db),
):
    """Get a generic platform setting (publicly readable if needed, or we can restrict)."""
    # for superadmin-temp-signup we need to read it publicly? 
    # Yes, the frontend checks if it is enabled before showing the form.
    result = await db.execute(
        select(PlatformSettings).where(PlatformSettings.id == key)
    )
    row = result.scalar_one_or_none()
    if not row:
...
async def set_generic_setting(
    key: str,
    data: Dict,
    current_user: Dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Set a generic platform setting (admin only)."""
    await _require_admin(current_user, db)
    result = await db.execute(
        select(PlatformSettings).where(PlatformSettings.id == key)
    )
    ...
    # if data has a 'settings' key, use it, else use data itself
    settings_data = data.get("settings", data)
    
    if row:
        row.settings = settings_data
    else:
        db.add(PlatformSettings(id=key, settings=settings_data))
    await db.flush()
    return {"success": True}
